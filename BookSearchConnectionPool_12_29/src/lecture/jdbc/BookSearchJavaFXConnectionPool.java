	...
	public void start(Stage PrimaryStage) throws Exception {
		...
		deleteBtn.setOnAction(e->{
			...
			try {
				...
				{
					con.commit();
					PreparedStatement newpstmt = con.prepareStatement(sql);
					
					newpstmt.setString(1, "%" + input+ "%");
					
					ResultSet newrs = newpstmt.executeQuery();
					
					ObservableList<BookVO> newlist = FXCollections.observableArrayList();
					
					while(newrs.next())
					{
						newlist.add(new BookVO(newrs.getString("bisbn"),newrs.getString("btitle"),newrs.getString("bauthor"),newrs.getInt("bprice")));
					}
