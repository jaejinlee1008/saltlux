...
public class BookSearchView extends Application{
	...
	public void start(Stage PrimaryStage) throws Exception {
		...
		pageColumn.setMinWidth(120);
		pageColumn.setCellValueFactory(new PropertyValueFactory<>("bpage"));
		
		TableColumn<BookExtraVO,String> translatorColumn = new TableColumn<>("TRANSLATOR"); 
		translatorColumn.setMinWidth(120);
		translatorColumn.setCellValueFactory(new PropertyValueFactory<>("btranslator"));
		
		TableColumn<BookExtraVO,String> supplementColumn = new TableColumn<>("SUPPLEMENT"); 
		supplementColumn.setMinWidth(120);
		supplementColumn.setCellValueFactory(new PropertyValueFactory<>("bsupplement"));
		
		TableColumn<BookExtraVO,String> publisherColumn = new TableColumn<>("PUBLISHER"); 
		...
		tableView.setRowFactory(e -> {
			...
			row.setOnMouseClicked(e1->{
				...
				if(e1.getClickCount()>1)
				{
					...
					extratableView.setItems(list);
					root.setCenter(extratableView);
				}
			});
