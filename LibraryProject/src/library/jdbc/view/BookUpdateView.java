...
import library.jdbc.controller.CheckCanUpdateBookController;
...
public class BookUpdateView {
	...
	public BookUpdateView(BorderPane booksearch, Scene scene, Stage primaryStage,BookVO book, TableView<BookVO> tableView, String keywordstr) {
		super();
	...
	private void apply(String btitle,String bauthor,String bprice,String bisbn)
	{
		CheckCanUpdateBookController controller = new CheckCanUpdateBookController();
		if(controller.getResult(bisbn,btitle,bauthor,bprice))
		{
		...
		{
			showDialog("알림", "책 정보 변경 실패 입력 정보를 다시 확인해주세요");
		}
	}
	...
	public BorderPane getRoot()
	{
		...
		bisbntf = new TextField(book.getBisbn());
		bisbntf.setPrefSize(200, 40);
		bisbntf.setDisable(true);
		
		
		btitle = new Text("책 제목");
		btitle.setWrappingWidth(50);
		
		btitletf = new TextField(book.getBtitle());
		...
		bauthortf = new TextField(book.getBauthor());
		...
		bpricetf = new TextField(Integer.toString(book.getBprice()));
		bpricetf.setPrefSize(200, 40);
		bpricetf.setOnAction(e->{
			apply(btitletf.getText(), bauthortf.getText(), bpricetf.getText(), book.getBisbn());
		});
